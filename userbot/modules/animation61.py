# https://github.com/vckyou/Geez-UserBot credits to @Adudin21

import asyncio


@bot.on(admin_cmd(pattern="gjm$"))
@bot.on(sudo_cmd(pattern="gjm$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    animation_interval = 0.05
    animation_ttl = range(200)
    event = await edit_or_reply(event, "gjm..")
    animation_chars = [
        "Kevin",
        "Adelina",
        "Jiaa",
...
async def _(event):
    ...
    animation_chars = [
        ...
        "😡",
        "😢",
        "‎**Repo nya Geez-Userbot**",
        "😁",
        "😧",
        "😡",
        "😢",
        "__**[Good to aku melihatmu....]**__(https://github.com/vckyou/Geez-UserBot)",
    ]
    for i in animation_ttl:
        await asyncio.sleep(animation_interval)
...
@bot.on(admin_cmd(pattern=f"adel$", outgoing=True))
@bot.on(sudo_cmd(pattern=f"adel$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    animation_interval = 2
    animation_ttl = range(6)
    event = await edit_or_reply(event, "Hey Kalian....")
    animation_chars = ["😁🏿", "😁🏾", "😁🏽", "😁🏼", "‎😁", "**Ga Jangan Maksa....**"]
    for i in animation_ttl:
        await asyncio.sleep(animation_interval)
        await event.edit(animation_chars[i % 6])


@bot.on(admin_cmd(pattern=f"kevin$", outgoing=True))
@bot.on(sudo_cmd(pattern=f"kevin$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    animation_interval = 2
    animation_ttl = range(12)
    event = await edit_or_reply(event, "Hey Maneh....")
    animation_chars = ["🐵", "🙉", "🙈", "🙊", "🖕‎🐵🖕", "**ini Kamu??....**"]
    for i in animation_ttl:
        await asyncio.sleep(animation_interval)
        await event.edit(animation_chars[i % 6])
...
@bot.on(admin_cmd(pattern=f"adudin$", outgoing=True))
@bot.on(sudo_cmd(pattern=f"adudin$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    animation_interval = 1
...
async def _(event):
    ...
    animation_chars = [
        ...
        "1️⃣",
        "0️⃣",
        "Geez-UserBot",(https://github.com/vckyou/Geez-UserBot)
    ]
    for i in animation_ttl:
        await asyncio.sleep(animation_interval)
...
\n\n📌** CMD ➥** `.gjm` | `.loading` | `.square` | `.up` | `.round` | `.hart` | `.anim` | `.adel` | `.adudin` | `.herber` | `.hand` | `.gsg` | `.theart`\
\n\n**USAGE   ➥  **cek aja\
"
    }
)
