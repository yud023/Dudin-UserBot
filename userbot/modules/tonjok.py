...
@register(outgoing=True, pattern=r'^.tonjok(:? |$)([1-8])?')
async def _(fry):
    await fry.edit("`Muka Udah Hina, Gua Bikin Lebih Hina Ah...😋`")
    level = fry.pattern_match.group(2)
    if fry.fwd_from:
        return
    if not fry.reply_to_msg_id:
        await fry.edit("`Mohon Balas Di Sticker Kontol`")
        return
    reply_message = await fry.get_reply_message()
    if not reply_message.media:
        await fry.edit("`Gambar tidak di dukung karena dia hina`")
        return
